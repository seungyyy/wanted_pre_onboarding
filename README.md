...
## 4. Click to Edit  ( ClickToEdit.js)
- 구현방법
    - `input value`의 초기값을 넣어 useState를 객체로 하여 필요한 내용들을 넣고 안의 값이 바뀌는 상태에 따라 변화도록 하였습니다.
    - `input focus`가 가면 label과 input value의 위치가 조정되게 css padding, vertical-align값을 조정하고 input아래 태그에 input value가 바뀔때 태그 안의 내용도 바뀔 수 있도록 하였습니다. 
      `useState의 false`값을 초기로 넣고 blur(마우스 포커스가 잃었을 때) 이전 상태가 아닌 현재 바뀐 input 내용을 아래 태그에 보일 수 있도록 하였습니다
    - 안의 내용이 바뀔때는 입력을 한 뒤 마우스의 포커스가 바뀌면 내용도 바뀌도록 하였습니다.
  
    
  - 이유  <br> 
  
    - input value가 바뀔 때 useState를 사용하여 onChange로 변화를 주었습니다. input 태그가 두개 이며 기능이 반복된 일이여서 초기에 값을 넣을때 객체 안에 key의 이름을 태그안에 name 과 동일하게 하여 이벤트 타겟의 이름이 같은 것을 찾아내서 해당 태그의 내용만 바뀔 수 있도록 하였습니다.

    - onBlur, onFocus, onChange 기능이 들어가야 구현할 수 있기 때문에 각각 포커스 되었을때 내용이 변화할때, 포커스를 잃었을때에 상황에 맞는 기능을 넣어 구현하였습니다.
  
  <br>

  - 구현하면서 어려웠던 점과 해결 방법 <br>
    - input 태그의 내용의 위치가 상단 가운데에 고정이 되어있고 포커스가 되었을때 가운데로 위치가 바뀌는 점
        - padding으로 하면 구현할 수 있다고 생각했는데 초반에 모든 상하좌우에 패딩을 주고 height도 값도 넣었더니 위치가 움직이지 않았지만 **고정된 height값을 제거 하고 padding-bottom에만 값 넣어** 해결하였습니다. 위치가 가운데로 가는 포커스가 되었을때 padding 값을 0으로 바꾼 후 height를 주고 포커스를 잃었을 경우에는 값을 다시 
        바꿔서 해결하였습니다.

  <br>

    - input value 값이 바뀔때 아래 태그의 해당내용이 보여지는데 
      input의 내용을 작성하는 순간이 아닌 포커스를 잃었을 경우에만 변경된 내용이 보여지게 하는 점 <br>
        - 해당 input에 포커스를 잃었을때에만 하단 내용이 바뀌고 그게 아닌 경우에는 이전 state의 내용을 담고 있어야 하는 점이 중요했습니다. <br>
        - useRef로 state의 값을 컴포넌트가 언마운트 될때까지 홀드할 수 있습니다. 렌더링이 발생해도 이전에 저장된 값을 유지합니다. useEffect를 사용해 아직 변경되지 않은 이전에 state값을 현재 state에 넣어 이전의 state값이 보여지도록 하였습니다. 하지만 text입력시에
      하단의 내용도 같이 움직이는 것은 해결하지는 못했습니다. 
  ...
  - 자세한 실행 방법
    - 이름, 나이 입력 창 안에 초기값이 나올 수 있도록 useState를 사용하여 초기값을 넣어 화면에 보여지도록 합니다.
    - 마우스의 포커스가 되면 onFocus를 사용하여 해당 입력창을 클릭하면 '이름'이라고 화면에 나오는 입력창 label의 위치가 입력창 가운데로 위치가 조정되고 입력창 안에 내용도 상단으로 고정이 되어있지만 클릭했을 경우 중앙으로 위치가 바뀔 수 있게 padding-bottom으로 아래의 공간을 만들었던 것을 지우고 height를 주어 중앙으로 오도록 하였습니다.
    - 입력창에 입력을 하면 하단의 내용이 이전 값을 가지고 포커스를 잃었을 경우에 입력한 값이 되도록 하였습니다. 
    useState의 false값을 초기에 넣고 onBlur가 될 때만 true로 하여 값이 true일때와 false일때 보여지는 태그 내용을 달리 하였습니다.